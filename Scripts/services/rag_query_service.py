...
try:
    ...
    try:
        ...
        class RedisCacheManager: # Dummy for fallback
            ...
            def close(self): pass
        # Dummy Prometheus metrics if import fails
...
except NameError: # In case prometheus_client is not available due to earlier import error
    logger.error("Prometheus metrics could not be defined due to missing prometheus_fastapi_instrumentator or its dependencies.")
    class DummyCounter: def labels(self, *args, **kwargs): return self; def inc(self): pass
    class DummyHistogram: def labels(self, *args, **kwargs): return self; def observe(self, val): pass
    CACHE_HITS_COUNTER, CACHE_MISSES_COUNTER = DummyCounter(), DummyCounter()
    QDRANT_QUERY_LATENCY, OLLAMA_LLM_LATENCY = DummyHistogram(), DummyHistogram()
...
async def startup_event():
    ...
    # ... (Qdrant, Embedding Model, LLM Client, Redis Cache init as before) ...
    try:
    ...
    instrumentator = Instrumentator(
        ...
        inprogress_name="rag_inprogress_requests",
        inprogress_labels=True,
    )
    instrumentator.instrument(app).expose(app) # Exposes /metrics endpoint
    # Register custom metrics if they were successfully defined
    if not isinstance(CACHE_HITS_COUNTER, DummyCounter): # Check if real metrics are available
        instrumentator.add(CACHE_HITS_COUNTER)
        instrumentator.add(CACHE_MISSES_COUNTER)
        instrumentator.add(QDRANT_QUERY_LATENCY)
        instrumentator.add(OLLAMA_LLM_LATENCY)
    logger.info("Prometheus instrumentation complete. Metrics available at /metrics (if not dummy).")
...
def _generate_cache_key(prefix: str, query: str, top_k: int, search_type_or_context: str) -> str:
    key_string = f"{prefix}:{query}:{top_k}:{search_type_or_context}"
...
async def _keyword_filter_search(query_text: str, top_k: int) -> List[SearchResult]:
    ...
    try:
        ...
        latency = time.time() - start_time
        QDRANT_QUERY_LATENCY.labels(query_type="keyword_filter").observe(latency)
...
async def _generate_llm_answer(query: str, context: str, model_name: str, query_request_details: QueryRequest) -> Optional[str]:
    ...
    if not query_request_details.force_no_cache and redis_cache and redis_cache.is_available():
        ...
        if cached_llm_answer:
            ...
            CACHE_HITS_COUNTER.labels(cache_type="llm_answer").inc()
...
async def perform_query(request: QueryRequest):
    # ... (rest of the perform_query function as previously defined, with cache checks)
    ...
    if not request.force_no_cache and redis_cache and redis_cache.is_available():
        ...
        if cached_search_data:
            ...
            CACHE_HITS_COUNTER.labels(cache_type="search_results").inc()
            cached_search_results_flag = True
        else:
            logger.info(f"Cache MISS for search results: key='{search_results_cache_key}'")
            CACHE_MISSES_COUNTER.labels(cache_type="search_results").inc()
    ...
    if request.generate_answer:
        ...
        llm_answer = await _generate_llm_answer(request.query, context_for_llm, LLM_MODEL_NAME, request) # Pass request for force_no_cache
...
async def health_check():
    # ... (health check as previously defined, ensure redis_cache.is_available() is part of it) ...
    q_ready = False
    if qdrant_client:
        try:
            qdrant_client.get_collection(collection_name=QDRANT_COLLECTION_NAME)
